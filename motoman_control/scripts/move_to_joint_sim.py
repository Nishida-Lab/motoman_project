...
def move_to_joint(end_pos, duration):
  ...
  pub = rospy.Publisher('/sia5_controller/command', JointTrajectory, queue_size=3)
...
def parse_args(args):
  ...
  try:
    ...
    # もし第1引数が"init"だったら初期姿勢を入れる
    if(args[0] == "init"):
      rospy.loginfo("初期姿勢に戻ります。")
      pos = [0] * len(get_cur_pos().position)
    else:
